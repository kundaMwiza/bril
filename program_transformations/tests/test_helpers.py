...
def compare_dicts(map_one: dict[str, Any], map_two: dict[str, Any]):
    def check_and_dispatch(iter_one, iter_two):
        assert type(v_two) == type(v_one), f"{type(v_one)=} is not equal to {type(v_two)=}"
        if isinstance(iter_one, dict):
            ...
            for i, (next_iter_one, next_iter_two) in enumerate(zip(iter_one, iter_two)):
                try:
                    ...
                    raise AssertionError(f"Index {i=} mismatch for {k_one=}, {next_iter_one=}, {next_iter_two=}") from e
        else:
            assert iter_one == iter_two, f"{iter_one == iter_two=} mismatch, {iter_one=}, {iter_two=}"
